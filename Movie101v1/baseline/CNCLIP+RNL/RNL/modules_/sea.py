...
class DenseCoAttn(nn.Module):
	...
	def forward(self, value1, value2, mask1=None, mask2=None):
		...
		if self.is_multi_head:
			weighted1, attn1 = qkv_attention(query1, query1, query1, mask=mask1, dropout=self.dropouts[0])
			weighted1 = weighted1.transpose(1, 2).contiguous()
			weighted2, attn2 = qkv_attention(query2, query2, query2, mask=mask2, dropout=self.dropouts[1])
			weighted2 = weighted2.transpose(1, 2).contiguous()
		else:
			weighted1, attn1 = qkv_attention(query1, query1, value1.unsqueeze(1), mask=mask1, 
				dropout=self.dropouts[0])
			weighted1 = weighted1.mean(dim=1)
			weighted2, attn2 = qkv_attention(query2, query2, value2.unsqueeze(1), mask=mask2, 
...
class NormalSubLayer2(nn.Module):

	def __init__(self, dim1, dim2, num_attn, dropout, dropattn=0):
		super(NormalSubLayer2, self).__init__()
	...
	def forward(self, data1, data2, mask1, mask2):
		...
		gate1 = self.linears_re[0](torch.cat([data1, weighted1], dim=2))
		gate2 = self.linears_re[1](torch.cat([data2, weighted2], dim=2))
		data1 = gate1 * data1 + (1 - gate1) * self.linears[0](torch.cat([data1, weighted1], dim=2))
		data2 = gate2 * data2 + (1 - gate2) * self.linears[1](torch.cat([data2, weighted2], dim=2))
