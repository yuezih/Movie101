...
class Charades(Dataset):
    ...
    def __getitem__(self, index):
        ...
        feats = load_feature(os.path.join(self.feature_path, '%s.npy' % vid), dataset='Charades')
...
class CharadesGCN(Dataset):
    ...
    def __getitem__(self, index):
        vid, duration, timestamps, sentence, words, id2pos = self.data[index]
        feats = load_feature(os.path.join(self.feature_path, '%s.npy' % vid), dataset='Charades')
        fps = feats.shape[0] / duration
