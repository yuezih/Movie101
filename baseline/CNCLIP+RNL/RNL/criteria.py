...
def inference_nms(predict_score, predict_windows, topn=5):
    rst_list = []
    scores_list = []
    IoU_thresh = 0.25
    picks = nms_temporal(predict_score, predict_windows, IoU_thresh)
    for index in picks[:topn]:
        pred_start = float(predict_windows[index][0])
        pred_end = float(predict_windows[index][1])
        score = float(predict_score[index])
        rst_list.append([pred_start,pred_end])
        scores_list.append(score)
    return rst_list, scores_list 
